...
if is_race = 0 
  THEN CALL LiveQualifying(ipDecimalJIT, beginUnixTimeJIT, curUnixTimeJIT);
ELSEIF is_race = 1 
  THEN CALL LiveRace(ipDecimalJIT, beginUnixTimeJIT, curUnixTimeJIT);
ELSE
  SELECT "ERROR";
END IF;
END // 
...
CREATE PROCEDURE LiveRace (in ipDecimalJIT int unsigned, in beginUnixTimeJIT int unsigned, in curUnixTimeJIT int unsigned)
SELECT
...
WHERE
  LapData.ipDecimal = ipDecimalJIT AND LapData.beginUnixTime = beginUnixTimeJIT AND LapData.curUnixTime = curUnixTimeJIT
ORDER BY
  LapData.carPosition;

CREATE PROCEDURE LiveQualifying (in ipDecimalJIT int unsigned, in beginUnixTimeJIT int unsigned, in curUnixTimeJIT int unsigned)
SELECT
  LapData.carPosition as 'NO.',
  LapData.driverName as "车手",
  teamName as "车队",
